...
public class ProductListActivity extends AppCompatActivity {
    ...
    public void populatedata() {
        ...
        if (first == 1) {
            ...
            Product product = new Product(1, "Beats Headphone",
                    "Active noise-cancellation, upto 60hr backup",
                    36.0, 53.534444, -113.490278);
            products.add(product);

            product = new Product(2	,"Universal Mobile charger"	," 3 metre cable, European standard",
                    31.2	,45.424722,	-75.695);
            products.add(product);

            product = new Product(3	,"Karoeke System"	,"In-built Battery",
                    75.0,49.260833,	-123.113889);
            products.add(product);

            product = new Product(4	,"Logitech mx530"	,"Bluetooth 5.0",
                    20.0,43.741667	,-79.373333);
            products.add(product);

            product = new Product(5	,"samsung gear"	,"Genuine gear made by samsung"	,
                    60.0,49.884444,	-97.146389);
            products.add(product);

            product = new Product(6,	"Google Specs"	,"UV ray protected"	,
                    50.0,53.534444	,-113.490278);
            products.add(product);

            product = new Product(	7,	"Gaming earbuds","Surround sound 7.1 audio, the best there is.",
                    15.0,45.424722,	-75.695);
            products.add(product);

            product = new Product(8,	"Iphone Full body cover"	,"9H glass, UV rays"	,
                    35.0,43.741667,	-79.373333);
            products.add(product);

            product = new Product(9,	"Lenovo y530 keyboard cover",	" Original cover made of silicon with photoshop keybindings."
                    ,50.0,49.260833,	-123.113889);
            products.add(product);

            product = new Product(10,	"logitech g450"	,"Wireless keyboard , bluetooth 5.1, supports all kinds of devices",
